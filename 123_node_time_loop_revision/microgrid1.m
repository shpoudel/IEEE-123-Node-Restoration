function [Aineq, bineq, mg_insP1, mg_insQ1, mg_insv1,mg_insP2, mg_insQ2, mg_insv2]=microgrid1(Aineq, bineq,mg1,power,loop_node1,yp1,f,path1,path2,nNodes,node)
    ...
    insert=1;      
    [P1eq,P11eq,indp1,indnode]=formeq(mg1,power,insert,G,loop_node1);
    [Q1eq,Q11eq,indq1,indnode]=formeq_reactive(mg1,power,insert,G,loop_node1);
    ...
    vec2P=indnode+nNodes*5+(size(loop_node1,2)-1)*5;
    vec2Q=indnode+nNodes*6+(size(loop_node1,2)-1)*6;
    ...
    from=nNodes*6+(size(loop_node1,2)-1)*5+1;
    to=nNodes*6+(size(loop_node1,2)-1)*6;
    s=size(f,1);
    step=1;
   [Aineq, bineq]=product_of_variables(Aineq,bineq, vec1P,vec2P,from ,to,s,step);
    %Product of variables for Q
    from=nNodes*7+(size(loop_node1,2)-1)*6+1;
    to=nNodes*7+(size(loop_node1,2)-1)*7;
    step=1;
   [Aineq, bineq]=product_of_variables(Aineq,bineq, vec1Q,vec2Q,from ,to,s,step);
    ...
    zi=zeros(nNodes,560);
    mg_insP1=[P11eq zi P1eq z2 z z2 z2 z             z2 z2 z z2 z2 z         z2 z2 z z2 z2 z           z2 z2 z z2 z2 z         z2 z2 z z2 z2 z];
    mg_insQ1=[Q11eq zi z2 Q1eq z  z2 z2 z            z2 z2 z z2 z2 z         z2 z2 z z2 z2 z           z2 z2 z z2 z2 z         z2 z2 z z2 z2 z];
    mg_insv1=[z zi Vp1 z1 Vq1 z1 Volt1 z2 z2 z     z z1 z z1 z z2 z2 z     z z1 z z1 z z2 z2 z       z z1 z z1 z z2 z2 z      z z1 z z1 z z2 z2 z];    
     %% 
     %% 
    ...
    %Later these values are used to calculate the voltage at the nodes with
    insert=1;
    [P2eq,P22eq,ind_p1,ind_node]=formeq_path_two(mg1,power,insert,G,loop_node1);
    [Q2eq,Q22eq,ind_q1,ind_node]=formeq_path_two_reactive(mg1,power,insert,G,loop_node1);
    for i=1:size(loop_node1,2)-1
    ...
    vec2P=ind_node+nNodes*8+(size(loop_node1,2)-1)*7;
    from=nNodes*9+7*(size(loop_node1,2)-1)+1;
    to=nNodes*9+(size(loop_node1,2)-1)*8;
    step=1;
    [Aineq, bineq]=product_of_variables(Aineq,bineq, vec1P,vec2P,from ,to,s,step); 
    %For Reactive power basedd on path #2
    vec2Q=ind_node+nNodes*9+(size(loop_node1,2)-1)*8;
    from=nNodes*10+(size(loop_node1,2)-1)*8+1;
    to=nNodes*10+(size(loop_node1,2)-1)*9;
    step=1;
    [Aineq, bineq]=product_of_variables(Aineq,bineq, vec1Q,vec2Q,from ,to,s,step); 
    %Voltage values according to path 1 needs to be calculated as follows,
    Volt2=zeros(125,125);
    ...
    z=zeros(nNodes,nNodes);    
    zi=zeros(nNodes,560);
    mg_insP2=[P22eq zi z2 z2 z P2eq z2 z           z2 z2 z z2 z2 z         z2 z2 z z2 z2 z           z2 z2 z z2 z2 z         z2 z2 z z2 z2 z];
    mg_insQ2=[Q22eq zi z2 z2 z  z2 Q2eq z          z2 z2 z z2 z2 z         z2 z2 z z2 z2 z           z2 z2 z z2 z2 z         z2 z2 z z2 z2 z];
    mg_insv2=[z zi z2 z2 z Vp2 z1 Vq2 z1 Volt2    z z1 z z1 z z2 z2 z    z z1 z z1 z z2 z2 z       z z1 z z1 z z2 z2 z     z z1 z z1 z z2 z2 z]; 
   end
